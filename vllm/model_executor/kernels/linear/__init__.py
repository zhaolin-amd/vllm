...
_POSSIBLE_NVFP4_KERNELS: dict[PlatformEnum, list[type[NvFp4LinearKernel]]] = {
    ...
    PlatformEnum.ROCM: [
        EmulationNvFp4LinearKernel,
        # Listed after emulation so it is never auto-selected (emulation always
        # wins in "auto" mode); the ROCm default stays emulation until the
        # FP4-MFMA path is validated. Opt in explicitly with
        # --linear-backend nvfp4_mxfp4_mfma (gfx950 / CDNA4 only).
        Nvfp4Mxfp4MfmaLinearKernel,
    ],
}
